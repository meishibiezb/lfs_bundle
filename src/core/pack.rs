...
use crate::core::manifest::{BundleManifest, FileEntry, sha256_file, write_manifest};
...
pub fn package_repository(req: &PackageRequest) -> Result<PackageSummary> {
    ...
    run_command(
        "git",
        &["update-ref", temp_ref.as_str(), req.end_commit.as_str()],
        Some(repo_path),
    )?;
    ...
    let delete_ref_result = run_command(
        "git",
        &["update-ref", "-d", temp_ref.as_str()],
        Some(repo_path),
    );
    ...
    let lfs_relative_paths = match run_command(
        ...
        &[
            ...
            "ls-files",
            "--long",
            req.start_commit.as_str(),
            req.end_commit.as_str(),
        ],
        Some(repo_path),
    ) {
        Ok(output) => {
            let stdout =
                String::from_utf8(output.stdout).context("git lfs output was not utf-8")?;
